...
import (
	...
	"github.com/Ncog-Earth-Chain/go-ncogearthchain/gossip/blockproc/drivermodule"
	"github.com/Ncog-Earth-Chain/go-ncogearthchain/gossip/blockproc/eventmodule"
	"github.com/Ncog-Earth-Chain/go-ncogearthchain/gossip/blockproc/evmmodule"
	"github.com/Ncog-Earth-Chain/go-ncogearthchain/gossip/blockproc/sealmodule"
...
func DefaultBlockProc(g ncogearthchain.Genesis) BlockProc {
	return BlockProc{
...
func NewService(stack *node.Node, config Config, store *Store, signer valkeystore.SignerI, blockProc BlockProc, engine forest.Consensus, dagIndexer *vecmt.Index) (*Service, error) {
...
func newService(config Config, store *Store, signer valkeystore.SignerI, blockProc BlockProc, engine forest.Consensus, dagIndexer *vecmt.Index) (*Service, error) {
	svc := &Service{
		config:             config,
...
// MakeProtocols constructs the P2P protocol definitions for `ncogearthchain`.
...
func (s *Service) Stop() error {
	defer log.Info("NCOGEarthChain service stopped")
	s.verWatcher.Stop()
	close(s.done)
	s.emitter.Stop()
	s.pm.Stop()
