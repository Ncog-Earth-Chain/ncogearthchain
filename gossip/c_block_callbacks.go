...
func (s *Service) GetConsensusCallbacks() forest.ConsensusCallbacks {
	return forest.ConsensusCallbacks{
...
func consensusCallbackBeginBlockFn(
	...
	return func(cBlock *forest.Block) forest.BlockCallbacks {
		wg.Wait()
		start := time.Now()
...
func spillBlockEvents(store *Store, block *inter.Block, network ncogearthchain.Rules) (*inter.Block, inter.EventPayloads) {
...
func mergeCheaters(a, b forest.Cheaters) forest.Cheaters {
	if len(b) == 0 {
		return a
	}
	if len(a) == 0 {
	...
	aSet := a.Set()
	merged := make(forest.Cheaters, 0, len(b)+len(a))
	for _, v := range a {
