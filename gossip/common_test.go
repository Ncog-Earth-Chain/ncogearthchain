...
func newTestEnv() *testEnv {
	genStore := makegenesis.FakeGenesisStore(genesisStakers, utils.ToNec(genesisBalance), utils.ToNec(genesisStake))
...
func (env *testEnv) ApplyBlock(spent time.Duration, txs ...*types.Transaction) (receipts types.Receipts) {
	...
	process := beginBlock(&forest.Block{
		Atropos: event.ID(),
	})
...
func (env *testEnv) callContract(
	...
	vmenv := vm.NewEVM(context, txContext, state, env.store.GetRules().EvmChainConfig(), ncogearthchain.DefaultVMConfig)
