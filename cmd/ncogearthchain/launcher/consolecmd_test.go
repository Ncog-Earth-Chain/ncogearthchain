...
const (
	ipcAPIs  = "abft:1.0 admin:1.0 dag:1.0 debug:1.0 nec:1.0 net:1.0 personal:1.0 rpc:1.0 sfc:1.0 txpool:1.0 web3:1.0"
	httpAPIs = "abft:1.0 dag:1.0 nec:1.0 rpc:1.0 sfc:1.0 web3:1.0"
)

// Tests that a node embedded within a console can be started up properly and
// then terminated by closing the input stream.
func TestConsoleWelcome(t *testing.T) {
	// Start a ncogearthchain console, make sure it's cleaned up and terminate the console
	cli := exec(t,
...
func TestIPCAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		...
		ipc = filepath.Join(ws, "ncogearthchain.ipc")
	}
	cli := exec(t,
		"--fakenet", "0/1", "--port", "0", "--maxpeers", "0", "--nodiscover", "--nat", "none",
		"--ipcpath", ipc)
...
func testAttachWelcome(t *testing.T, cli *testcli, endpoint, apis string) {
	// Attach to a running ncogearthchain node and terminate immediately
